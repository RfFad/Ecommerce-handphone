...
if (isset($_GET["action"])){
    ...
    elseif($_GET["action"] == "beli"){
        ...
        $query=mysqli_query($koneksi, "SELECT * FROM login WHERE  email='$_SESSION[email]' " );
        $d =mysqli_fetch_array($query);
        $id_pembeli=$d['id_user'];
        $alamat= $_POST['alamat'];
        
        $ql = mysqli_query($koneksi, "INSERT INTO checkout (id_checkout,alamat,id_user,tgl,total) VALUES ('$alamat','$id_pembeli','".date("Y-m-d")."','$total')");
    }
    $id_trans = mysqli_insert_id($koneksi);
        foreach($_SESSION["cart"] as $key => $value){
            $id_prod = $value['id_pembeli'];
            $qty = $value['item_quantity'];
            $sql = "INSERT INTO detail VALUES ('','$id_trans','$qty' ,'$id_prod')";
                        ...
                        <div class="card  border-0">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" name="alamat"   placeholder="your addres" required>
                                </div>
                                <div class="form-group">
                                    <label for="name" class="small text-muted mb-1">NO HP</label>
                                    <input type="text" class="form-control form-control-sm" name="no_hp"   placeholder="your nomer" required>
                                </div>
                                <div class="form-group">
